...
export function PromptMedia({ prompt }: { prompt: PromptCardItem }) {
  const thumbs = [prompt, ...promptCards.filter((item) => item.title !== prompt.title)].slice(0, 7);
  ...
  return (
    <section className="rounded-2xl border border-[#202746] bg-[#080d19] p-2">
      <div className="relative h-[520px] overflow-hidden rounded-xl border border-[#243052] bg-[#0b1222]">
        <CropImage className={`absolute inset-0 h-full w-full scale-[2.7] ${prompt.crop}`} />
      ...
      <div className="mt-2 grid grid-cols-7 gap-2">
        {thumbs.map((item, index) => (
          ...
          >
            <CropImage className={`absolute inset-0 h-full w-full ${item.crop}`} />
            {index > 0 ? <Play className="absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 text-white/80" size={18} fill="currentColor" /> : null}
          </button>
        ))}
      </div>
    </section>
